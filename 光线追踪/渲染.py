	...
	def paint(self):
		for i, j in self.m目标.m图像:
			...
			for k in range(self.m采样数):
				...
				v光线 = self.m投射.get_ray(u, v)
				v颜色 += self.m光线染色.ray_color(self.m场景, v光线)
...
class C染色_物体颜色:
	def __init__(self):
		pass
	@ti.func
	def ray_color(self, a场景, a光线):
		v光线位置 = a光线.m位置
		v光线方向 = a光线.m方向
		v光线颜色 = t向量3(0, 0, 0)
		v碰撞, v交点, v交点法线, v前面, v物体颜色, v材质 = a场景.ray_hit(C光线(v光线位置, v光线方向))
		if v碰撞:
			v光线颜色 = v物体颜色
		return v光线颜色
@ti.data_oriented
...
class C染色_光线追踪:
	def __init__(self):
		pass
	@ti.func
	def ray_color(self, a场景, a光线):	#光线追踪
		v当前深度 = 0
		v光线位置 = a光线.m位置
		v光线方向 = a光线.m方向
		v光线颜色 = t向量3(1, 1, 1)
